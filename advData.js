...
import advModel from './api/adventures/adventureModel';
...
const adventures = [
    ...
    },
    {
      ...
      "tests": [
		...
		{
		"name": "Eat some cheese",
		"threshold": 0,
		"statRequired": "cheese",
		"onSuccess": "door1CheesePass",
		"onFail": "door1CheeseFail"
		}
      ]
    },
	{
	"id": "door1CheesePass",
	"text": "You eat some cheese. It's tasty, despite its age!",
	...
	{
	"id": "door1CheeseFail",
	"text": "You somehow fail at eating cheese. How DID you do that, anyway?",
	"previous": "openDoorTestResult",
	"next": [],
	"statGain": [],
	"tests": []
...
export const loadAdventures = () => {advModel.find({}).remove(function() {
	advModel.collection.insert(adventures, (err, docs)=>{
		if(err){
			console.log('failed to load Adventures data');
		}
		else{
		console.info('${adventures.length} adventures were successfully stored.');	
		}
	})
});	
}